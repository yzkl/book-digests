...
async def setup_chapters(session: AsyncSession) -> None:
    ...
    c2 = models.Chapter(id=2, book_id=2, number=1)
    c3 = models.Chapter(id=3, book_id=3, number=1, title="other-chapter")
    c4 = models.Chapter(id=4, book_id=4, number=1)
...
def assert_model_equal(expected: models.Digest, result: Digest) -> None:
...
async def test_create_digest_happy_path(
    ...
    result = await services.create_digest(
...
@pytest.mark.asyncio
async def test_create_digest_raises_on_nonexistent_chapter(
    reset_db, testing_session: AsyncSession
) -> None:
    await setup(testing_session)

    with pytest.raises(
        RelatedEntityDoesNotExistError, match="Chapter with id=1000 not found"
    ):
        await services.create_digest(
            DigestCreate(chapter_id=1000, paragraph_number=1, digest="test-digest"),
            testing_session,
        )
...
# -----------------
@pytest.mark.asyncio
async def test_get_digest_happy_path(reset_db, testing_session: AsyncSession) -> None:
    data = await setup(testing_session)

    result = await services.get_digest(1, testing_session)

    assert_model_equal(data["x1"], result)


@pytest.mark.asyncio
async def test_get_digest_raises_on_nonexistent_digest(
    reset_db, testing_session: AsyncSession
) -> None:
    await setup(testing_session)

    with pytest.raises(EntityDoesNotExistError, match="Digest with id=1000 not found"):
        await services.get_digest(1000, testing_session)
...
# -----------------
@pytest.mark.asyncio
async def test_get_digests_happy_path(reset_db, testing_session: AsyncSession) -> None:
    data = await setup(testing_session)

    result = await services.get_digests(testing_session)
...
@pytest.mark.asyncio
async def test_get_digests_ordering(reset_db, testing_session: AsyncSession) -> None:
    await setup(testing_session)
...
@pytest.mark.asyncio
async def test_get_digests_returns_empty_sequence(
    reset_db, testing_session: AsyncSession
) -> None:
    result = await services.get_digests(testing_session)
...
@pytest.mark.asyncio
async def test_get_digests_custom_page(reset_db, testing_session: AsyncSession) -> None:
    await setup(testing_session)

    result = await services.get_digests(testing_session, page=1000)

    assert len(result) == 0


@pytest.mark.asyncio
async def test_get_digests_custom_size(reset_db, testing_session: AsyncSession) -> None:
    data = await setup(testing_session)

    result = await services.get_digests(testing_session, size=2)
...
@pytest.mark.asyncio
async def test_get_digests_custom_page_custom_size(
    reset_db, testing_session: AsyncSession
) -> None:
    data = await setup(testing_session)

    result = await services.get_digests(testing_session, page=2, size=3)
...
async def test_update_digest_happy_path(
    ...
    result = await services.update_digest(
...
async def test_update_digest_raises_on_nonexistent_digest(
    ...
    with pytest.raises(EntityDoesNotExistError, match="Digest with id=1000 not found"):
        await services.update_digest(
            1000, DigestUpdate(digest="updated-digest"), testing_session
        )


@pytest.mark.asyncio
async def test_update_digest_raises_on_nonexistent_chapter(
    reset_db, testing_session: AsyncSession
) -> None:
    await setup(testing_session)
    ...
    with pytest.raises(
        ...
        await services.update_digest(
            1, DigestUpdate(chapter_id=1000, digest="updated-digest"), testing_session
        )
...
async def test_delete_digest_happy_path(
    ...
    result = await services.delete_digest(1, testing_session)
...
async def test_delete_digest_raises_on_nonexistent_digest(
    ...
    with pytest.raises(EntityDoesNotExistError, match="Digest with id=1000 not found"):
        await services.delete_digest(1000, testing_session)
