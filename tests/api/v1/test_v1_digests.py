...
async def setup_chapters(session: AsyncSession) -> None:
    ...
    x1 = models.Chapter(id=1, book_id=1, number=1)
    x2 = models.Chapter(id=2, book_id=2, number=1)
    x3 = models.Chapter(id=3, book_id=3, number=1)
    x4 = models.Chapter(id=4, book_id=4, number=1)
    x5 = models.Chapter(id=5, book_id=5, number=1)
    session.add_all([x1, x2, x3, x4, x5])
    await session.commit()
...
async def setup(session: AsyncSession) -> None:
    ...
    x2 = models.Digest(id=2, chapter_id=1, paragraph_number=1, digest="some-digest")
    x3 = models.Digest(id=3, chapter_id=1, paragraph_number=1, digest="other-digest")
    x4 = models.Digest(id=4, chapter_id=1, paragraph_number=1, digest="another-digest")
    x5 = models.Digest(id=5, chapter_id=1, paragraph_number=1, digest="last-digest")
    session.add_all([x1, x2, x3, x4, x5])
    await session.commit()
...
@pytest.mark.asyncio
async def test_create_digest_returns_201(
    ...
    response = await async_client.post(
        PREFIX,
        json={"chapter_id": 1, "paragraph_number": 1, "digest": "test-digest"},
    )
...
@pytest.mark.asyncio
async def test_create_digest_returns_422_on_nonexistent_chapter(
    reset_db, testing_session: AsyncSession, async_client: AsyncClient
) -> None:
    ...
    assert "Chapter with id=1000 not found" in response.json()["message"]


@pytest.mark.asyncio
async def test_get_digest_returns_200(
    reset_db, testing_session: AsyncSession, async_client: AsyncClient
) -> None:
    await setup(testing_session)
...
@pytest.mark.asyncio
async def test_get_digest_returns_404_on_nonexistent_digest(
    reset_db, testing_session: AsyncSession, async_client: AsyncClient
) -> None:
    await setup(testing_session)
    ...
    assert "Digest with id=1000 not found" in response.json()["message"]


@pytest.mark.asyncio
async def test_get_digests_returns_data_and_pagination(
    reset_db, testing_session: AsyncSession, async_client: AsyncClient
) -> None:
    await setup(testing_session)
...
@pytest.mark.asyncio
async def test_get_digests_respects_page_and_size(
    reset_db, testing_session: AsyncSession, async_client: AsyncClient
) -> None:
    await setup(testing_session)
...
@pytest.mark.asyncio
async def test_get_digests_on_out_of_range_page(
    reset_db, testing_session: AsyncSession, async_client: AsyncClient
) -> None:
    await setup(testing_session)
...
@pytest.mark.asyncio
async def test_update_digest_returns_200(
    reset_db, testing_session: AsyncSession, async_client: AsyncClient
) -> None:
    await setup(testing_session)

    response = await async_client.put(PREFIX + "1", json={"digest": "updated-digest"})

    assert response.status_code == 200


@pytest.mark.asyncio
async def test_update_digest_returns_404_on_nonexistent_digest(
    reset_db, testing_session: AsyncSession, async_client: AsyncClient
) -> None:
    await setup(testing_session)

    response = await async_client.put(
        PREFIX + "1000", json={"digest": "updated-digest"}
    )
...
@pytest.mark.asyncio
async def test_update_digest_returns_422_on_nonexistent_chapter(
    reset_db, testing_session: AsyncSession, async_client: AsyncClient
) -> None:
    await setup(testing_session)

    response = await async_client.put(
        PREFIX + "1", json={"chapter_id": 1000, "title": "updated-digest"}
    )
...
@pytest.mark.asyncio
async def test_delete_digest_returns_200(
    reset_db, testing_session: AsyncSession, async_client: AsyncClient
) -> None:
    await setup(testing_session)
...
@pytest.mark.asyncio
async def test_delete_digest_returns_404_on_nonexistent_digest(
    reset_db, testing_session: AsyncSession, async_client: AsyncClient
) -> None:
    await setup(testing_session)
