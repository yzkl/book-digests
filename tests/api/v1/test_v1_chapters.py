...
@pytest.mark.asyncio
async def test_create_chapter_returns_201(
...
@pytest.mark.asyncio
async def test_create_chapter_returns_422_on_nonexistent_book(
    reset_db, testing_session: AsyncSession, async_client: AsyncClient
) -> None:
...
@pytest.mark.asyncio
async def test_get_chapter_returns_200(
    reset_db, testing_session: AsyncSession, async_client: AsyncClient
) -> None:
    await setup(testing_session)
...
@pytest.mark.asyncio
async def test_get_chapter_returns_404_on_nonexistent_chapter(
    reset_db, testing_session: AsyncSession, async_client: AsyncClient
) -> None:
    await setup(testing_session)
...
@pytest.mark.asyncio
async def test_get_chapters_returns_data_and_pagination(
    reset_db, testing_session: AsyncSession, async_client: AsyncClient
) -> None:
    await setup(testing_session)
...
@pytest.mark.asyncio
async def test_get_chapters_respects_page_and_size(
    reset_db, testing_session: AsyncSession, async_client: AsyncClient
) -> None:
    await setup(testing_session)
...
@pytest.mark.asyncio
async def test_get_chapters_on_out_of_range_page(
    reset_db, testing_session: AsyncSession, async_client: AsyncClient
) -> None:
    await setup(testing_session)
...
@pytest.mark.asyncio
async def test_update_chapter_returns_200(
    reset_db, testing_session: AsyncSession, async_client: AsyncClient
) -> None:
    await setup(testing_session)

    response = await async_client.put(PREFIX + "1", json={"title": "updated-chapter"})

    assert response.status_code == 200


@pytest.mark.asyncio
async def test_update_chapter_returns_404_on_nonexistent_chapter(
    reset_db, testing_session: AsyncSession, async_client: AsyncClient
) -> None:
    await setup(testing_session)

    response = await async_client.put(
        PREFIX + "1000", json={"title": "updated-chapter"}
    )
...
@pytest.mark.asyncio
async def test_update_chapter_returns_422_on_nonexistent_book(
    reset_db, testing_session: AsyncSession, async_client: AsyncClient
) -> None:
    await setup(testing_session)

    response = await async_client.put(
        PREFIX + "1", json={"book_id": 1000, "title": "updated-chapter"}
    )
...
@pytest.mark.asyncio
async def test_delete_chapter_returns_200(
    reset_db, testing_session: AsyncSession, async_client: AsyncClient
) -> None:
    await setup(testing_session)
...
@pytest.mark.asyncio
async def test_delete_chapter_returns_404_on_nonexistent_chapter(
    reset_db, testing_session: AsyncSession, async_client: AsyncClient
) -> None:
    await setup(testing_session)
