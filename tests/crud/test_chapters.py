...
def assert_model_equal(expected: models.Chapter, result: models.Chapter) -> None:
...
async def test_create_chapter_happy_path(
    ...
    result = await chapters.create_chapter(
...
# ------------------
@pytest.mark.asyncio
async def test_find_chapter_happy_path(reset_db, testing_session: AsyncSession) -> None:
    data = await setup(testing_session)

    result = await chapters.find_chapter(1, testing_session)

    assert_model_equal(data["x1"], result)


@pytest.mark.asyncio
async def test_find_chapter_raises_on_nonexistent_chapter(
    reset_db, testing_session: AsyncSession
) -> None:
    await setup(testing_session)

    with pytest.raises(EntityDoesNotExistError, match="Chapter with id=1000 not found"):
        await chapters.find_chapter(1000, testing_session)
...
# ------------------
@pytest.mark.asyncio
async def test_find_chapters_happy_path(
    reset_db, testing_session: AsyncSession
) -> None:
    data = await setup(testing_session)

    result = await chapters.find_chapters(testing_session)
...
@pytest.mark.asyncio
async def test_find_chapters_ordering(reset_db, testing_session: AsyncSession) -> None:
    await setup(testing_session)
...
@pytest.mark.asyncio
async def test_find_chapters_returns_empty_sequence(
    reset_db, testing_session: AsyncSession
) -> None:
    result = await chapters.find_chapters(testing_session)
...
@pytest.mark.asyncio
async def test_find_chapters_custom_page(
    reset_db, testing_session: AsyncSession
) -> None:
    await setup(testing_session)

    result = await chapters.find_chapters(testing_session, page=1000)

    assert len(result) == 0


@pytest.mark.asyncio
async def test_find_chapters_custom_size(
    reset_db, testing_session: AsyncSession
) -> None:
    data = await setup(testing_session)

    result = await chapters.find_chapters(testing_session, size=2)
...
@pytest.mark.asyncio
async def test_find_chapters_custom_page_custom_size(
    reset_db, testing_session: AsyncSession
) -> None:
    data = await setup(testing_session)

    result = await chapters.find_chapters(testing_session, page=2, size=3)
...
async def test_update_chapter_happy_path(
    ...
    result = await chapters.update_chapter(
...
async def test_delete_chapter_happy_path(
    ...
    result = await chapters.delete_chapter(1, testing_session)
...
async def test_delete_chapter_raises_on_nonexistent_chapter(
    ...
    with pytest.raises(EntityDoesNotExistError, match="Chapter with id=1000 not found"):
        await chapters.delete_chapter(1000, testing_session)
...
# ------------------
@pytest.mark.asyncio
async def test_count_all_chapters_happy_path(
    reset_db, testing_session: AsyncSession
) -> None:
    await setup(testing_session)

    result = await chapters.count_all_chapters(testing_session)

    assert result == 4


@pytest.mark.asyncio
async def test_count_all_chapters_returns_zeroes(
    reset_db, testing_session: AsyncSession
) -> None:
    result = await chapters.count_all_chapters(testing_session)
